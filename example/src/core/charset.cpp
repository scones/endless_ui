...
namespace core {
  ...
  void charset::render_text(std::string const& text, float x, float y, core::t_vec2ui32 resolution) {
    glEnable(GL_BLEND);
    static const unsigned int points_layout_id = 0;
    static const unsigned int uvs_layout_id = 1;
    float sx = 2.0f / static_cast<float>(resolution.x);
    float sy = 2.0f / static_cast<float>(resolution.y);
...
//    glBufferData(GL_ARRAY_BUFFER, sizeof(core::t_vec2f) * uvs.size(), uvs.data(), GL_STATIC_DRAW);
  ...
  void charset::render_text_ortho(std::string const& text, std::uint32_t x, std::uint32_t y, core::t_vec2ui32 resolution, std::uint32_t alignment) {
    static const unsigned int points_layout_id = 0;
    static const unsigned int uvs_layout_id = 1;
    ...
    // projection matrix (identity)
    glm::highp_mat4x4 projection_matrix(glm::ortho<float>(0, resolution.x, resolution.y, 0, -1.0f, 1.0e20f));
    glProgramUniformMatrix4fv(m_shader.get_program_name(), m_projection_matrix_location_id, 1, GL_FALSE, &projection_matrix[0][0]);
